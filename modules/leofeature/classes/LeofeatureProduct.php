...
class LeofeatureProduct extends ProductControllerCore
{
    ...
    public function getTemplateVarProduct2($id_product, $id_product_attribute)
    {
        ...
        if(!isset($product['cart_quantity'])){
            ...
            $product_cart = $this->context->cart->getProducts();
            foreach ($product_cart as $product_c) {
                if($product_c['id_product'] == $product['id_product'] && $product_c['id_product_attribute'] == $product['id_product_attribute']){
                    $product['cart_quantity'] = $product_c['cart_quantity'];
                }
            }
        }
