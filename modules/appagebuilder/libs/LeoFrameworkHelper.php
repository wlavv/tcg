                ...
                if (!Cache::isStored($cacheId)) {
                    ...
                    if ($databaseResults) {
                        foreach ($databaseResults as $record) {
                            $hookNamesByAlias[strtolower($record['alias'])] = $record['name'];
                        }
                    }
                    Cache::store($cacheId, $hookNamesByAlias);
                } else {
                    $hookNamesByAlias = Cache::retrieve($cacheId);
                }
                if (isset($hookNamesByAlias[strtolower($hook_name)])) {
                    // return the canonical name (?)
                    $retro_hook_name = $hookNamesByAlias[strtolower($hook_name)];
                }

                $alias = array_search($hook_name, $hookNamesByAlias);
                if ($alias === false) {
                    $retro_hook_name = '';
                } else {
                    $retro_hook_name = $alias;
                }
            } else {
            ...
            foreach ($module_list as $array) {
                ...
                if (array_key_exists($moduleInstance->id, $this->overrideHooks)) {
                    if (version_compare(Configuration::get('PS_VERSION_DB'), '8.0.0', '>=')) {
                        ...
                        if (isset($hookNamesByAlias[strtolower($this->overrideHooks[$moduleInstance->id])])) {
                            // return the canonical name (?)
                            $ohook = $hookNamesByAlias[strtolower($this->overrideHooks[$moduleInstance->id])];
                        }

                        $alias = array_search($this->overrideHooks[$moduleInstance->id], $hookNamesByAlias);
                        if ($alias === false) {
                            $ohook = '';
                        } else {
                            $ohook = $alias;
