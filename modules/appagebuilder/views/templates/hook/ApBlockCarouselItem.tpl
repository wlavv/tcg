			...
			{foreach from=$sliders item=slider name="sliders"}
				...
				<div class="{$scolumn|escape:'html':'UTF-8'}">
					...
					{if isset($slider.image) && !empty($slider.image)}
						<img class="img-fluid{if $aplazyload} lazy{/if}" {if $aplazyload} data-src{else}src{/if}="{$slider.image|escape:'html':'UTF-8'}" alt="{if isset($slider.title)}{$slider.title|escape:'html':'UTF-8'}{/if}" loading="lazy"/>
					{else}
						{if isset($slider.image_link) && !empty($slider.image_link)}
							<img class="img-fluid{if $aplazyload} lazy{/if}" {if $aplazyload} data-src{else}src{/if}="{$slider.image_link|escape:'html':'UTF-8'}" alt="{if isset($slider.title)}{$slider.title|escape:'html':'UTF-8'}{/if}" loading="lazy"/>
						{/if}
					{/if}
