            ...
            {foreach from=$widthList item=itemWidth}
                ...
                <li>
                    <a href="javascript:void(0);" data-width="{$itemWidth|escape:'html':'UTF-8'}" class="number-column">
                        <span class="width-val ap-w-{if $itemWidth|strpos:"."|escape:'html':'UTF-8'}{$itemWidth|replace:'.':'-'|escape:'html':'UTF-8'}{else}{$itemWidth|escape:'html':'UTF-8'}{/if}">{$itemWidth|escape:'html':'UTF-8'}/12 - ( {math equation="x*100" x=$colwidth format="%.2f"} % )</span>
                    </a>
                </li>
            {/foreach}
        </ul>
