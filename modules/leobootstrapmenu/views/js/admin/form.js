...
jQuery(document).ready(function(){
	...
	if($('#megamenu').length)
	{
		$("html, body").animate({ scrollTop: $('#megamenu').offset().top - 150 }, 500);
	}
...
jQuery(document).ready(function(){
    ...
    $('body').on('click', '#availableItems option', function(){
    	$('[name="quicksavemenu"]').val($(this).val());
    })
    $('body').on('click', '#leoAddMenuItem', function(e){
    	e.preventDefault();
    	if($('[name="quicksavemenu"]').val()){
    		$('#leoMenuItem').submit();
    	}
    })
});