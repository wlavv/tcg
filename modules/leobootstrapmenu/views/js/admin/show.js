...
function updateField(action, value, is_lang)
{
	...
	if (is_lang == true) {
		// remove element not field (name is number or empty string)
		var list_field_lang = $('#list_field_lang').val().split(',');
		var new_list_field_lang = '';
		for	(var i = 0; i< list_field_lang.length; i++){
		  if(isNaN(parseInt(list_field_lang[i])) && list_field_lang[i]){
		    new_list_field_lang += new_list_field_lang ? ','+list_field_lang[i] : list_field_lang[i];
		  }
		}
		$('#list_field_lang').val(new_list_field_lang+',');
	}
