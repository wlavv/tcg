...
define('LEO_PS_SEARCH_MAX_WORD_LENGTH', 15);
...
define('LEO_PREG_CLASS_SEARCH_EXCLUDE', '\x{0}-\x{2c}\x{2e}-\x{2f}\x{3a}-\x{40}\x{5b}-\x{60}\x{7b}-\x{bf}\x{d7}\x{f7}\x{2b0}-' .
...
define('LEO_PREG_CLASS_NUMBERS', '\x{30}-\x{39}\x{b2}\x{b3}\x{b9}\x{bc}-\x{be}\x{660}-\x{669}\x{6f0}-\x{6f9}' .
...
define('LEO_PREG_CLASS_PUNCTUATION', '\x{21}-\x{23}\x{25}-\x{2a}\x{2c}-\x{2f}\x{3a}\x{3b}\x{3f}\x{40}\x{5b}-\x{5d}' .
 ...
 */
define('LEO_PREG_CLASS_CJK', '\x{3041}-\x{30ff}\x{31f0}-\x{31ff}\x{3400}-\x{4db5}\x{4e00}-\x{9fbb}\x{f900}-\x{fad9}');
...
class ProductSearch
{
    ...
    public static function sanitize($string, $id_lang, $indexation = false, $iso_code = false)
    {
        ...
        $string = preg_replace('/([' . LEO_PREG_CLASS_NUMBERS . ']+)[' . LEO_PREG_CLASS_PUNCTUATION . ']+(?=[' . LEO_PREG_CLASS_NUMBERS . '])/u', '\1', $string);
        $string = preg_replace('/[' . LEO_PREG_CLASS_SEARCH_EXCLUDE . ']+/u', ' ', $string);
        ...
        if (in_array($iso_code, array('zh', 'tw', 'ja')) && function_exists('mb_strlen')) {
            ...
            foreach (explode(' ', $string) as $mb_word) {
                if (Tools::strlen(Tools::replaceAccentedChars($mb_word)) == mb_strlen(Tools::replaceAccentedChars($mb_word))) {
                    $letters .= $mb_word . ' ';
                } elseif (str_word_count($string) > 0) {// check kanji letter
                    $letters .= $mb_word . ' ';
                } else {
                    $symbols .= $mb_word . ' ';
                }
    ...
    public static function find($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position', $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null, $id_cate = null)
    {
        ...
        // check kanji letter
        $min_words = ((str_word_count($expr) == 0) && in_array($context->language->iso_code, array('zh', 'tw', 'ja'))) ? 1 : (int) Configuration::get('PS_SEARCH_MINWORDLEN');

        foreach ($words as $key => $word) {
            if (!empty($word) && (Tools::strlen($word) >= $min_words || Tools::strlen($expr) >= $min_words)) {
                ...
                $intersect_array[] = 'SELECT si.id_product
                                        ...
                                        WHERE sw.id_lang = ' . (int) $id_lang . '
                                                ...
                                                AND sw.word LIKE
                                        ' . ($word[0] == '-' ? ' \'' . $start_search . pSQL(Tools::substr($word, 1, LEO_PS_SEARCH_MAX_WORD_LENGTH)) . $end_search . '\'' : ' \'' . $start_search . pSQL(Tools::substr($word, 0, LEO_PS_SEARCH_MAX_WORD_LENGTH)) . $end_search . '\''
                        );

                if ($word[0] != '-') {
                    $score_array[] = 'sw.word LIKE \'' . $start_search . pSQL(Tools::substr($word, 0, LEO_PS_SEARCH_MAX_WORD_LENGTH)) . $end_search . '\'';
                }
            } else {
                unset($words[$key]);
            }
        ...
        if (Group::isFeatureActive()) {
            ...
            $sql_groups = (count($groups) ? 'AND cg.`id_group` IN (' . implode(',', $groups) . ')' : '');
        }
    ...
    protected static function fillProductArray(&$product_array, $weight_array, $key, $value, $id_lang, $iso_code)
    {
        if (strncmp($key, 'id_', 3) && isset($weight_array[$key])) {
            ...
            foreach ($words as $word) {
                if (!empty($word)) {
                    $word = Tools::substr($word, 0, LEO_PS_SEARCH_MAX_WORD_LENGTH);
