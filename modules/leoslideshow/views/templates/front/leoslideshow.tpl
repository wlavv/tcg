...
<div class="bannercontainer banner-{$sliderParams.slider_class}{if $sliderParams.group_class} {$sliderParams.group_class|escape:'html':'UTF-8'}{/if}" style="padding: {$sliderParams.padding|escape:'html':'UTF-8'};margin: {$sliderParams.margin|escape:'html':'UTF-8'};">
	<div class="iview {$class_group}" style="height:{if $sliderParams.height}{$sliderParams.height|intval}{else}780{/if}px">
		{if $sliders}
			{foreach from=$sliders key=k item=slider}
				{if $slider.video.active}
					...
					<div data-leo_image="{$slider.thumbnail|escape:'html':'UTF-8'}"
						...
						<iframe src="{$slider.video.videoURL|escape:'html':'UTF-8'}?title=0&amp;byline=0&amp;portrait=0;api=1{if $slider.video.videoauto}&autoplay=1&loop=1&mute{if $slider.video.usevideo != 'youtube'}d{/if}=1{/if}" width="100%" height="100%" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen{if $slider.video.videoauto} allow="autoplay"{/if}></iframe>
					</div>
					<!-- SLIDE VIDEO END -->
				{else}
					...
					<div class="slide_config {if isset($slider.data_link)&& $slider.data_link}data-link{/if}"
						{if $slider.main_image != ''} data-leo_image="{$slider.image|escape:'html':'UTF-8'}"{/if}
								...
								<div class="tp-caption {if $layer.layer_link}data-link{/if}{if $layer.layer_class} {$layer.layer_class|escape:'html':'UTF-8'}{/if}" 
									...
									{if $layer.layer_type == "video"}<!-- LAYER VIDEO BEGIN -->
										{if $layer.layer_video_type == "vimeo"}
											<iframe src="http://player.vimeo.com/video/{$layer.layer_video_id|escape:'html':'UTF-8'}?title=0&amp;byline=0&amp;portrait=0;api=1" width="{$layer.layer_video_width|escape:'html':'UTF-8'}" height="{$layer.layer_video_height|escape:'html':'UTF-8'}" ></iframe>
										{else}
											<iframe src="http://www.youtube.com/embed/{$layer.layer_video_id|escape:'html':'UTF-8'}" width="{$layer.layer_video_width|escape:'html':'UTF-8'}" height="{$layer.layer_video_height|escape:'html':'UTF-8'}"></iframe>
										{/if}
									{/if}<!-- LAYER VIDEO END -->
...
{if isset($load_from) AND $load_from == 'leoelements_backend'}

{elseif isset($load_from) AND $load_from == 'leoelements_frontend'}
	if(typeof (leoslideshow_list_functions) == 'undefined'){
	    var leoslideshow_list_functions = [];
	}
    leoslideshow_list_functions.push(function(){
{elseif isset($load_from_appagebuilder) AND $load_from_appagebuilder == 1}
...
//		strips: {$sliderParams.strips|default:'20'|intval},
		...
		loading_show : "{if isset($sliderParams.loading_show)}{$sliderParams.loading_show}{else}none{/if}",
		...
		onAfterLoad: function() 
		{
			...
			{/if}
			if($('.{$class_group} .iviewSlider').children().eq(0).hasClass('iview-video')){
			  var iframe = $('.{$class_group} .iviewSlider').children().eq(0).find('iframe');
