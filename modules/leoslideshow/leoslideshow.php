...
class LeoSlideshow extends Module
{
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitUpdateModule')) {
            ...
            $this->context->smarty->assign(array(
                'confirmations' => $this->l('Update and Correct Module is successful'),
            ));
    ...
    public function correctDataGroup()
    {
        ...
        if ($id_group) {
            ...
            if (Validate::isLoadedObject($mod_group)) {
                ...
                if ($params) {
                    $mod_group->params = LeoSlideshowSlide::base64Encode(json_encode($params));
                    $mod_group->save();
                }
                ...
                foreach ($sliders as $slider) {
                    ...
                    if (Validate::isLoadedObject($mod_slide)) {
                        ...
                        if ($tmp) {
                            $mod_slide->params = LeoSlideshowSlide::base64Encode(json_encode($tmp));
                        }
                        ...
                        foreach ($mod_slide->video as $key => $value) {
                            ...
                            if ($tmp) {
                                $tmp_slide[$key] = LeoSlideshowSlide::base64Encode(json_encode($tmp));
                            }
                        }
                        if ($tmp_slide) {
                    ...
                    foreach ($languages as $language) {
                        if ($language['id_lang'] != $default_lang) {
                            foreach ($tmp as $key => $val) {
                                if ($key == 'layersparams') {
                                    $layers_params = json_decode(LeoSlideshowSlide::base64Decode($val), true);
                                    if (is_array($layers_params)) {
                                        ...
                                        $mod_slide->layersparams[$language['id_lang']] = LeoSlideshowSlide::base64Encode(json_encode($layers_params));
                                    }
                                } else {
                                    # module validation
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => &$slide) {
            ...
            $image_add = json_decode(LeoSlideshowSlide::base64Decode($slide['layersparams']));

            //display image
            if ((!(Tools::substr($slides[$key]['image'], 0, 7) == 'http://')) && (!(Tools::substr($slides[$key]['image'], 0, 8) == 'https://')) && (!(Tools::substr($slides[$key]['image'], 0, 10) == 'data:image'))    ) {
                if($slides[$key]['image'] != '') {
                    $slide['slide_img'] = $this->img_url . $slides[$key]['image'];
                } else {
                    $slide['slide_img'] = isset($image_add[0]) ? $this->img_url.$image_add[0]->layer_content : '';
                }
            } else {
                $slide['slide_img'] = $slides[$key]['image'];
            }
        }
        ...
        $this->group_data = array_merge($this->group_data, json_decode(LeoSlideshowSlide::base64Decode($mod_group->params), true));
//        $arrayParam['secure_key'] = $this->secure_key;
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'options' => array(
                            'query' =>  array(
                                array('id' => 'same', 'name' => $this->l('Same Window')),
                                array('id' => 'new', 'name' => $this->l('New Window')),
                            ),
                            'id' => 'id',
                            'name' => 'name',
                        )
    ...
    public function getSliderFieldsValues()
    {
        ...
        if (isset($this->mod_current_slide->id) && $this->mod_current_slide->id) {
            ...
            $slide = $this->mod_current_slide;
            $this->slider_data = array_merge($this->slider_data, json_decode(LeoSlideshowSlide::base64Decode($slide->params), true));
        }
        ...
        foreach ($languages as $lang) {
            ...
            $fields['thumbnail'][$lang['id_lang']] = Tools::getValue('thumbnail_'.(int)$lang['id_lang'], isset($slide->thumbnail[$lang['id_lang']]) ? $slide->thumbnail[$lang['id_lang']] : '');
            $fields['title'][$lang['id_lang']] = Tools::getValue('title_'.(int)$lang['id_lang'], isset($slide->title[$lang['id_lang']]) ? $slide->title[$lang['id_lang']] : '');
            $fields['link'][$lang['id_lang']] = Tools::getValue('link_'.(int)$lang['id_lang'], isset($slide->link[$lang['id_lang']]) ? $slide->link[$lang['id_lang']] : '');
            if ($slide->video) {
                if ($slide->video[(int)$lang['id_lang']]) {
                    foreach (json_decode(LeoSlideshowSlide::base64Decode($slide->video[(int)$lang['id_lang']]), true) as $key => $value) {
...
//                    foreach ($layer as $k => &$l) {
        ...
        if ($this->mod_current_slide->video) {
            foreach ($this->mod_current_slide->video as $key => $val) {
                $video = json_decode(LeoSlideshowSlide::base64Decode($val), true);
                $slide_back[$key] = '';
    ...
    public function renderGroupList()
    {
        ...
        foreach ($groups as $key => $group) {
            if ($group['id_leoslideshow_groups'] == Tools::getValue('id_group') || (!Tools::getValue('id_group') && !Tools::isSubmit('addNewGroup') && $group['id_leoslideshow_groups'] == Configuration::get('LEOSLIDESHOW_GROUP_DE'))) {
                ...
                $params = json_decode(LeoSlideshowSlide::base64Decode($group['params']), true);
                if ($params) {
                    $group_result = array();
                }
    ...
    public function renderGroupConfig()
    {
        ...
        $fields_form[0]['form'] = array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'select',
                        'label' => $this->l('Loadding'),
                        'name' => 'group[loading_show]',
                        'options' => array(
                            'query' => LeoSlideshowConfig::getLoaddingBarOption(),
                            'id' => 'id',
                            'name' => 'name',
                        ),
                    ),
...
//            'exportLink' => Context::getContext()->link->getAdminLink('AdminLeoSlideshow').'&exportGroup=1',
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitGroup')) {
            ...
            if (!Tools::getValue('id_group')) {
                if (!$mod_group->title) {
                    $errors[] = $this->displayError($this->l('Group title is required.'));
                } elseif (!$mod_group->add()) {
    ...
    private function _prepareHook($hook_name)
    {
        ...
        $slider_params = json_decode(LeoSlideshowSlide::base64Decode($group['params']), true);
        // echo '<pre>';
        ...
        foreach ($sliders as $key => $slider) {
            $slider['layers'] = array();
            $slider['params'] = array_merge($this->slider_data, json_decode(LeoSlideshowSlide::base64Decode($slider['params']), true));
            $slider['layersparams'] = json_decode(LeoSlideshowSlide::base64Decode($slider['layersparams']), true);
            $slider['video'] = json_decode(LeoSlideshowSlide::base64Decode($slider['video']), true);
            ...
            if ($slider['image'] == '') {
                $slider['image'] = __PS_BASE_URI__.'modules/leoslideshow'.'/views/img/blank.gif';
            }
    ...
    public function converParams($old_params = '')
    {
        ...
        if ($old_params != '') {
            ...
            $result = LeoSlideshowSlide::base64Encode(json_encode($data));
        }
        return $result;
    }
    # ******************** SLIDE ********************
    ...
    public function slideProcessAjax()
    {
        ...
        if (Tools::getValue('action') && Tools::getValue('action') == 'submitslider') {
            ...
            $mod_slide->params = LeoSlideshowSlide::base64Encode(json_encode(Tools::getValue('slider')));
            ...
            foreach ($languages as $language) {
                ...
                $mod_slide->video[$language['id_lang']] = LeoSlideshowSlide::base64Encode(json_encode($video));
                ...
                if (Tools::getIsset('layers_'.$language['id_lang'])) {
                    ...
                    foreach ($layers as $key => $value) {
                        ...
                        $value['layer_caption'] = str_replace(array('\'', '\"'), array("'", '"'), $value['layer_caption']);
                    ...
                    $mod_slide->layersparams[$language['id_lang']] = LeoSlideshowSlide::base64Encode(json_encode($layersparams->layers));
                } else {
                    ...
                    if (!Tools::getValue('id_slide') && isset($tmp_data['layersparams']) && $tmp_data['layersparams']) {
                        ...
                        $mod_slide->layersparams[$language['id_lang']] = LeoSlideshowSlide::base64Encode(json_encode($tmp_data['layersparams']));
                    } else {
            ...
            if (!Tools::getValue('id_slide')) {
                ...
                if (LeoSlideshowConfig::getPermission() == false) {
                    ...
                    if ($number_group >= 2) {
                        $result = array('error' => 2, 'text' => 'I am so sorry, you are using free version of Leo slideshow module, could you please update module to pro version to use this function');
                        die(json_encode($result));
                    }
                }
                # add default image
        ...
        if (!$this->isCached($tpl, $this->getCacheId($group_id.'_'.$this->name))) {
            ...
            foreach ($sliders as $key => $slider) {
                ...
                if ($slider_params['image_cropping']) {
                    ...
                    if ($slider['image'] && file_exists($this->img_path.$slider['image'])) {
                        $slider['main_image'] = $this->img_url.$slider['image'];
                        $slider['image'] = $this->img_url.$slider['image'];
            ...
            $this->smarty->assign(array(
                ...
                'sliderFullwidth' => $slider_fullwidth,
                'sliderImgUrl' => $this->img_url,
                'load_from' => ( isset($this->load_from) && $this->load_from )  ? $this->load_from : $this->name,
    ...
    {
        if (version_compare(Configuration::get('PS_VERSION_DB'), '8.0.0', '>=')) {
            $cms = $this->smarty->getTemplateVars('cms');
        } else {
    ...
    private function exportGroup()
    {
        ...
        call_user_func('header', 'Content-Disposition: Attachment; filename=export_group_'.Tools::getValue('id_group').'_'.time().'.txt');
        call_user_func('header', 'Pragma: no-cache');
        die(LeoSlideshowSlide::base64Encode(json_encode($group)));
    }
