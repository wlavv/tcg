...
class ConnectionConfig 
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('shards', $value)) {
            $this->_usedProperties['shards'] = true;
            $this->shards = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\Doctrine\Dbal\ConnectionConfig\ShardConfig($v) : $v, $value['shards']);
            unset($value['shards']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['shards'])) {
            $output['shards'] = array_map(fn ($v) => $v instanceof \Symfony\Config\Doctrine\Dbal\ConnectionConfig\ShardConfig ? $v->toArray() : $v, $this->shards);
        }
